...
| 功能 | 说明 |
|---|---|
| 多格式导入 | CSV / TSV / JSON / TXT（`账号----密码`）；后缀不认识也能按内容识别；还能**直接粘贴**导入 |
| 缺少 SteamID64 也能用 | 只有账号+密码时，查询阶段自动调用 Valve `IAuthenticationService` 解析出 SteamID64（不需要 Steam Guard 验证码） |
| 一键自动登录 | 结束当前 Steam → 改写 `loginusers.vdf` + 注册表 → 启动 Steam → **自动把密码填进登录窗口**（Win32 输入注入，参考同类工具做法） |
| 登录时写入 Steam 设置 | 登录首页改为库、关闭推销广告弹窗、好友窗口不自动弹出（**保持在线状态**） |
