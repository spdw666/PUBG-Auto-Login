...
| 一键自动登录 | 结束当前 Steam → 启动 Steam → **仅在 Steam 安装目录内的 `steam.exe` / `steamwebhelper.exe` 登录窗口填入密码**；已有 SteamID64 时还会改写 `loginusers.vdf` 与注册表；登录进行中会拒绝重复启动，避免双击拉起两个 Steam 线程 |
...
| 跨版本数据继承 | 从 v3.1.3 起，账号库与保存的 Key 统一保存在当前 Windows 用户的 LocalAppData。**下载新版直接打开就能用**：程序在常见旧位置后台查找账号库，并将旧资料**安全合并**进当前库；当前账号、备注、查询结果与登录记录优先保留，旧库只补齐缺失字段。扫描总计最多 25 秒或 12,000 个目录，并跳过 AppData、链接/联接点；非常规位置可点「迁移旧版数据」 |
