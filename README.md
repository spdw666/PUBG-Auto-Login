...
覆盖：账号库质量排序与自动继承判定（空库/缺密码才自动换、账号更少的旧库不许顶替）、增量导入不清空账号名/备注、占位账号合并保留密码、CSV 64 KiB 截断仍识别编码、loginusers.vdf 空格缩进可解析、登录窗口本地化标题、命令行不含密码、一次性处理 4 万个 id、CSV/JSON/TXT 三种导入（含无表头账号+密码 CSV、同账号占位合并）、密码 DPAPI 加解密回读、明文不入库、CSV/TXT/JSON 导出→再导入往返（含密码含分隔符）、JSON 真实进度、筛选清空选择、重复登录拦截、登录计时与排序、版本比较、封禁结果写入与库存封禁筛选、分页、API Key 加解密、未解析 SteamID64 的配置写入保护。返回 0 即通过。
