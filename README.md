...
| 批量查询封禁 | 用你自己的 Steam Web API Key 查 VAC / 游戏封禁 / 社区封禁 / 库存封禁，并给出 PUBG 关联判断；界面实时显示已完成数量与百分比进度 |
...
覆盖：CSV/JSON/TXT 三种导入（含无表头账号+密码 CSV、同账号占位合并）、密码 DPAPI 加解密回读、明文不入库、CSV/TXT/JSON 导出→再导入往返（含密码含分隔符）、JSON 真实进度、筛选清空选择、重复登录拦截、封禁结果写入与库存封禁筛选、分页、API Key 加解密、未解析 SteamID64 的配置写入保护。返回 0 即通过。
