...
| 跨版本数据继承 | 从 v3.1.3 起，账号库与保存的 Key 统一保存在当前 Windows 用户的 LocalAppData。**下载新版直接打开就能用**：程序在常见旧位置后台查找账号库，并将旧资料**安全合并**进当前库；无论旧库或当前库哪一方还保留账号占位行，均会合并到真实 SteamID64 行，避免重复账号和漏补密码。扫描总计最多 25 秒或 12,000 个目录，并跳过 AppData、链接/联接点；非常规位置可点自动迁移 |
