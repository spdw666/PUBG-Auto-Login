...
| 跨版本数据继承 | 从 v3.1.3 起，账号库与保存的 Key 统一保存在当前 Windows 用户的 LocalAppData；v3.1.4 会后台自动查找并迁移旧版数据，找不到时仍可手动选择旧版账号库 |
...
- v3.1.2 及更早版本把上述文件放在 EXE 旁边；v3.1.4 会先检查常见目录，再在**后台**查找当前 Windows 用户目录内任意深度的固定文件名 `steam_ban_accounts.sqlite3`，验证其包含账号表后自动迁移最近使用的一份。扫描不会阻塞界面，也不会读取其它文件内容。若旧数据在另一个 Windows 用户、外接盘或其它磁盘，仍可点击「迁移旧版数据」选择该文件；迁移前会备份当前库。
